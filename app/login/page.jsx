...
export default function LoginPage() {
  ...
  return (
    <div className="flex h-screen w-full bg-[#fdfaf6]">
      <div className="hidden md:flex w-1/2 bg-[#1a1a1a] p-16 flex-col justify-center">
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center p-8">
        <form onSubmit={handleLogin} className="w-full max-w-sm bg-white p-8 rounded-2xl border border-[#e5e0d6] shadow-xl">
          <h2 className="text-2xl font-bold text-[#1a1a1a] mb-6 text-center">Acesso Restrito</h2>
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <input type="email" value={email} onChange={e=>setEmail(e.target.value)} required className="w-full pl-12 p-3 bg-gray-50 rounded-lg outline-none" placeholder="Email (admin@licitacoes.ia)" />
            </div>
            <div className="relative">
              <Lock className="absolute left-4 top-3 text-gray-400" size={20}/>
              <input type="password" value={password} onChange={e=>setPassword(e.target.value)} required className="w-full pl-12 p-3 bg-gray-50 rounded-lg outline-none" placeholder="Senha (estrategia123)" />
            </div>
            <button type="submit" disabled={loading} className="w-full bg-[#1a1a1a] text-white p-3 rounded-lg font-bold flex justify-center">
