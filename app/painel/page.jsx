...
```/g, '').trim();
    ...
    } catch (e) { throw new Error("Falha ao montar inteligência."); }
  }
  return data.result;
};
    ...
    <div className="flex flex-col items-center gap-1">
      <div className="relative w-20 h-20">
        <svg width="80" height="80" className="-rotate-90">
          ...
          <circle cx="40" cy="40" r={r} fill="none" stroke={color} strokeWidth={6} strokeDasharray={c} strokeDashoffset={offset} strokeLinecap="round" className="transition-all duration-1000" />
        </svg>
...
const AuditReport = ({ report }) => {
  ...
  return (
    <div className="bg-white border border-[#e5e0d6] rounded-xl overflow-hidden mb-4 w-full shadow-sm flex flex-col">
      ...
      <div className="p-4 md:p-5 bg-[#fcf9f2] border-b border-[#e5e0d6]">
        <p className="text-[14.5px] md:text-[15px] text-[#111318] leading-relaxed font-medium"><strong className="text-[#c9a84c]">↳ Resumo Tático: </strong>{report.resumoExecutivo}</p>
      </div>
          ...
          <div className="flex flex-col gap-4">
            {(report.vulnerabilidades || []).map((v, i) => (
              <div key={i} className="relative border border-[#f5c0c3] bg-[#fff9f9] rounded-xl p-4 md:p-5 overflow-hidden">
                ...
                <span className="bg-red-50 text-red-700 text-[11px] font-bold px-2 py-1 rounded border border-red-100 uppercase inline-block mb-3">{v.gravidade}</span>
                <h4 className="text-base font-bold text-[#111318] mb-2">{v.titulo}</h4>
          ...
          <div className="flex flex-col gap-4">
            {(report.oportunidadesOfensivas || []).map((op, i) => (
              <div key={i} className="relative border border-[#c9a84c] bg-[#fdf6e3] rounded-xl p-4 md:p-5 overflow-hidden">
                ...
                <h4 className="text-base font-bold text-[#111318] mb-2 flex items-center gap-2"><Target size="{18}" className="text-[#a68832]"/> {op.titulo}</h4>
                <p className="text-[14.5px] text-[#5c4716] leading-relaxed mb-4 font-medium">{op.descricao}</p>
                <span className="inline-flex items-center gap-1.5 bg-white text-[#8a6d2f] text-[13px] font-bold px-2 py-1 rounded border border-[#e5e0d6]"><Gavel size="{14}"/> {op.fundamento}</span>
              </div>
            ))}
          </div>
        )}
                 ...
                 <div className="flex flex-col gap-2">
                   {(cat.itens || []).map((item, j) => (
                     <div key={j} className="flex gap-3 p-3 border border-gray-200 rounded-lg bg-gray-50 items-start">
                       <CheckCircle size="{18}" className="{item.alerta" ? "text-[#c9a84c] shrink-0 mt-0.5" : "text-gray-300 mt-0.5"}/>
        ...
        {tab === "prazos" && (
           <div className="flex flex-col gap-3">
             {(report.prazos || []).map((p, i) => (
               <div key={i} className="flex flex-col md:flex-row justify-between p-4 border border-gray-200 rounded-xl bg-white gap-3">
                 <div className="flex items-center gap-4">
                   <Clock size="{20}" className="text-[#111318]"/>
                   <div><p className="font-bold text-[15px]">{p.tipo}</p><p className="font-mono text-[13px] text-gray-500">{p.data}</p></div>
                 </div>
                 <span className="bg-gray-100 text-gray-700 text-[11px] font-bold px-2 py-1 rounded uppercase self-start md:self-auto">{p.urgencia}</span>
               </div>
             ))}
           </div>
        )}
        {tab === "minuta" && (
           <div className="bg-[#fdfaf6] border border-[#e5e0d6] rounded-xl p-5 text-[14.5px] leading-loose text-[#111318] font-serif whitespace-pre-wrap">{report.minutaImpugnacao}</div>
        )}
      </div>
      <div className="bg-gray-50 border-t border-[#e5e0d6] p-4 flex gap-3 items-start">
        ...
        <p className="text-[12px] leading-relaxed text-gray-500 font-medium">Aviso Legal: Ferramenta de inteligência probabilística. Valide dados com profissional habilitado.</p>
      </div>
    </div>
  );
};
...
export default function MainApp() {
  ...
  useEffect(() => { if (session?.user?.id) fetchHistory(); }, [session]);
  ...
  const saveAuditToDatabase = async (fileName, textContext, report) => {
    ...
    try {
      await fetch('/api/historico', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ fileName, reportData: report, fullText: textContext }) });
      fetchHistory();
  ...
  const loadPastAudit = (audit) => {
    setMenuOpen(false);
    setAppState({ fileName: audit.fileName, context: audit.fullText || "", messages: [{ role:'user', type:'text', text:`📄 Histórico: **${audit.fileName}**` }, { role:'assistant', type:'audit', report: audit.reportData }] });
  };
  ...
  const handleFile = async (e) => {
    ...
    setMenuOpen(false); setIsProcessing(true);
    setAppState({ fileName: file.name, context: '', messages: [{ role:'user', type:'text', text:`📄 Avaliando: **${file.name}**` }] });
    try {
  ...
  const handleSend = useCallback(async () => {
    ...
    const userText = input; setInput(''); setIsProcessing(true);
  ...
  if (status === "loading") return <div className="h-screen w-full flex items-center justify-center bg-[#111318]"><Loader2 className="animate-spin text-[#c9a84c]" size="{48}"/></div>;
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden bg-[#fdfaf6] font-sans">
      ...
      <aside className={`fixed md:relative top-0 left-0 h-full w-[280px] bg-[#111318] flex flex-col z-50 transform transition-transform duration-300 md:translate-x-0 ${menuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-6 flex items-center justify-between border-b border-white/5">
          <div className="flex items-center gap-3">
            ...
            <span className="text-white text-[19px] font-bold">Soluções.IA</span>
          </div>
          <button className="md:hidden text-gray-400 p-2" onClick={() => setMenuOpen(false)}><X size="{24}"/></button>
        </div>
        <div className="p-5">
          <button onClick={() => fileInputRef.current?.click()} disabled={!pdfjsReady || isProcessing} className="w-full bg-[#c9a84c] text-[#111318] font-bold py-4 rounded-xl flex items-center justify-center gap-2 hover:brightness-110 disabled:opacity-50">
        ...
        <div className="flex-1 overflow-y-auto p-4 border-t border-white/5">
          <p className="text-gray-500 text-[11px] font-bold uppercase mb-3 flex items-center gap-2"><History size="{14}"/> Arquivo ({historyList.length})</p>
          {loadingHistory ? <div className="flex justify-center p-4"><Loader2 className="animate-spin text-gray-500" size="{20}"/></div> : historyList.map(audit => (
            <button key={audit.id} onClick={() => loadPastAudit(audit)} className="w-full text-left p-3 hover:bg-white/5 rounded-lg transition-colors flex flex-col gap-1">
              <span className="text-gray-300 text-[13px] font-medium truncate w-full">{audit.fileName}</span>
            </button>
          ))}
        </div>
        <div className="p-5 border-t border-white/5 bg-black/40">
           <div className="flex items-center gap-3 mb-4 px-2">
              ...
              <div><p className="text-white text-[13px] font-medium truncate">{session?.user?.email}</p></div>
           </div>
           <button onClick={() => signOut({ callbackUrl: '/login' })} className="flex items-center gap-3 text-red-400 font-bold text-[14px] px-2 py-2 w-full rounded-lg hover:bg-red-950/30"><LogOut size="{16}"/> Encerrar</button>
        </div>
      </aside>
      <main className="flex-1 flex flex-col min-w-0 bg-[#fdfaf6]">
        <div className="h-16 md:h-[72px] bg-white border-b border-[#e5e0d6] flex items-center px-4 md:px-8 shrink-0 gap-4">
           ...
           <div>
             <h2 className="text-[#111318] font-bold text-[15px]">{appState.fileName || 'Centro de Comando'}</h2>
           </div>
        </div>
        <div className="flex-1 overflow-y-auto p-4 md:p-8">
           {isEmpty ? (
              <div className="h-full flex flex-col items-center justify-center text-center max-w-xl mx-auto">
                 <div className="w-24 h-24 bg-[#c9a84c] rounded-3xl flex items-center justify-center mb-8"><Scale size="{48}" className="text-[#111318]"/></div>
                 <h1 className="text-[32px] font-extrabold text-[#111318] mb-4">Vantagem Decisiva</h1>
                 <p className="text-gray-600 text-[15px] mb-8">Insira o edital para varredura completa.</p>
              </div>
           ) : (
              <div className="flex flex-col gap-6 max-w-4xl mx-auto w-full pb-4">
                 {appState.messages.map((msg, i) => (
                    <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                       {msg.role === 'user' ? (
                          <div className="bg-[#111318] text-white px-5 py-4 rounded-2xl max-w-[80%] text-[15px]">{msg.text}</div>
                       ) : msg.type === 'audit' ? (
                          <AuditReport report="{msg.report}"/>
                       ) : (
                          <div className="bg-white border border-[#e5e0d6] px-5 py-5 rounded-2xl w-full text-[15px]" dangerouslySetInnerHTML={{ __html: renderMarkdown(msg.text) }} />
                       )}
                    </div>
                 ))}
                 {isProcessing && <div className="flex items-center gap-3 bg-white p-4 rounded-xl"><Loader2 className="animate-spin text-[#c9a84c]"/><span className="text-[15px] font-bold text-[#8a6d2f]">Processando...</span></div>}
                 <div ref={chatEndRef} />
              </div>
           )}
        </div>
        <div className="p-4 md:p-6 bg-white border-t border-[#e5e0d6] shrink-0">
           <div className="max-w-4xl mx-auto flex gap-3">
              <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if(e.key === 'Enter') handleSend(); }} disabled={isProcessing || !appState.context} className="flex-1 bg-[#fdfaf6] border border-[#e5e0d6] rounded-xl px-5 py-4 outline-none" placeholder="Questione o edital..." />
              <button onClick={handleSend} disabled={!input.trim() || isProcessing} className="bg-[#111318] text-white w-14 h-14 rounded-xl flex items-center justify-center"><Send size="{24}"/></button>
           </div>
        </div>
      </main>
    </div>
